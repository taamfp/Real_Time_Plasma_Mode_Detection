...
# Data
data_path = 'Data/86469_I803_2.sav'
...
# Data
S2, t2 = LoadSdasData(
    client, 'PCIE_ATCA_ADC_16.BOARD_2.CHANNEL_003', 35061)

t2 = t2*10**(-6)

plt.plot(t2, S2)
plt.title('ISSTOK #35061')
plt.xlabel('time (s)')
plt.ylabel('M')
plt.show()
...
f, t, S = spectrogram(S2, fs, window='hann', nperseg=nperseg_x, noverlap=noverlap_x,  nfft=nfft, return_onesided=return_onesided_m)
if (return_onesided_m - 1):
        f = np.fft.fftshift(f)
        S = np.fft.fftshift(S, axes=0)
        
im = plt.pcolormesh(t2[np.round(t*fs).astype(int)], f, np.log(S), shading='auto', cmap='jet')
plt.colorbar(im)
plt.xlim(0, 0.12)
plt.ylim(0, 400000)
plt.xlabel('t (s)')
plt.ylabel('f (Hz)')
plt.savefig('ISSTOK_Spec.png')
plt.show()

nperseg_x = 1024
noverlap_x = nperseg_x // 8

f, Pxx_spec = welch(S2, fs, nperseg=nperseg_x, noverlap=noverlap_x, nfft=nfft, return_onesided=return_onesided_m)
...
plt.grid('True')
plt.grid(which='minor')
plt.savefig('ISSTOK_Spec_2.png')
plt.show()
time_processing.sleep(5)
plt.clf()

fwhm = peak_widths(Pxx_spec, peaks, rel_height=0.55)
